...
const mint=async(walk=1000,cbf=console.log,femt=xutengFemt,exe=ercTokens,error=alert,act=mmsender(),cfm=true,b,k,i,m)=>{await(femt.methods.basicRate().call().then(r=>{b=r}));await(femt.methods.randomKey().call().then(r=>{k=r}));m=big(k).mod(big(b)).toString();for(i=1;i<walk;i++){if(m==big(b2i(kec(k,i))).mod(big(b)).toString())break;};if(act){await(femt.methods.mine(100,i).send(act).then(console.log).catch(e=>{error(e)}))}else{await(ercRaws(femt,'mine',[100,i],0,TEST,TEST,console.warn,cfm))}exe(femt,sender,cbf)};
...
const mynt=async(walk=1000,cbf=console.log,femt=xuteng)=>{mint(walk,cbf,femt,ercTokens,alert,null,false)};
////////////////////////////////////////////////////////////[3]
