...
;if(nid=='xga-')Object.keys(a).forEach(function(t){if(a[t]&&avalid(a[t][0]))deaddrLottoGame(a[t][0],TEST,function(err,result){if(err||!result||!result.dat||!result.dat.name){mw(t,nolink);}else{mw(t,'<button\tclass="axut\u0020bxut"\tonclick="window.open(\''+xgaUrl+a[t][0]+'\');">'+result.dat.name+'</button>');}});/**RPC**/;});/**keys**/;
