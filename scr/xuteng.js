...
const txSend=function(divG,divF,divS,cbf=console.log,cfm=true){return(sendingFunc.estimateGas({from:sender,value:s2wHex(sendingEth)}).then(gas=>{estgas=gas;gasfee=fromGwei(estgas*txgwei);dw(divG,estgas);dw(divF,gasfee);if(cfm&&!accepted(divS))return(null);console.warn('GAS',estgas,'FEE',gasfee);return(web3.eth.getTransactionCount(sender))}).then(nonce=>{nonce=nonce.toString(16);console.warn('NONCE',nonce);return(web3.eth.sendSignedTransaction(txraw(sendingAbi,nonce,sendingEth,0)))}))};
