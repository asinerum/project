...
////////////////////////////////////////////////////////////
const btnXut=function(tag,nid,a,n,i,t,v){if(!nid)nid='xut'+HYPHEN;a={};n=document.getElementsByTagName(tag?tag:STAR);for(i=0;i<n.length;i++){t=n[i].className;v=n[i].getAttribute('value');if(t&&t.indexOf(nid)===0){a[t]=[t.split(HYPHEN)[1],Number(v)];}}Object.keys(a).forEach(function(t){if(a[t]&&avalid(a[t][0]))xuteng.methods.incomeOf(a[t][0]).call(function(err,result){if(err){mw(t,'<i\tclass="fa\u0020fa-trash"></i>');}else{mw(t,'<button\tclass="axut\tbxut"\tonclick="incomeOf(\''+a[t][0]+'\',\'of-'+a[t][0]+'\');"><i\tclass="fa\u0020fa-flask"></i></button><button\tclass="axut"><span\tclass="of-'+a[t][0]+'">'+w2s(result)+'</span></button><button\tclass="axut\u0020bxut"\tonclick="mm_transfer(\''+a[t][0]+'\',g2(\'to-'+a[t][0]+'\'));"><i\tclass="fa\u0020fa-diamond"></i></button><input\ttype="text"\tclass="axut\u0020bxut"\tid="to-'+a[t][0]+'"\tsize="3"\tvalue="'+a[t][1]+'"/>');}});});};
