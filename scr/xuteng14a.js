...
;if(nid=='eth-')Object.keys(a).forEach(function(t){if(a[t]&&avalid(a[t].address)){window.ethref=setInput({ref:a[t].ref});mw(t,'<button\tclass="axut\u0020bxut"\tonclick="mm_sendeth(\''+a[t].address+'\','+a[t].value+',window.ethref,\''+a[t].index+HYPHEN+a[t].address+'\');"\tid="'+a[t].index+HYPHEN+a[t].address+'">ETH&nbsp;'+a[t].value+'</button>');}});/**keys**/;
