...
const createSimpleRef=function(frm='ethref',uts=lotnow().lotstamp,min=0,max=1,typ=TYPES.draft,dat={},odd=[0],hdiv='hash',alias=ZEROADDR){typ=Number(typ);uts=Number(uts);min=Number(min);max=Number(max);showLoad(hdiv);mm_sendeth(alias,0,setInput({typ,frm,uts,min,max,odd,dat}),hdiv);};
