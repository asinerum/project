////////////////////////////////////////////////////////////
const MBALANCE = 'balanceOf';
const MPROGGET = 'markets';
const MPROGPAY = 'acquire';
const MPROGRAM = 'post';
const MPROGSTP = 'unpost';
////////////////////////////////////////////////////////////
let _days = 'days';
let _martMoney = 'NEMT7';
let _progMoney = 'GEMT7';
////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////
let _errCoins = `"CoinsNotEnough"`;
let _errDepos = `"TokensNotEnough"`;
let _errIdNot = `"IdNotAvailable"`;
let _errInput = `"InputNotValid"`;
let _errInvst = `"TradeNotAllowed"`;
let _errItNon = `"TradeItemNotFit"`;
let _errItNot = `"TradeItemNotExist"`;
let _errOwner = `"ActionNotAllowed"`;
let _errValim = `"ValueExceedsLimit"`;
let _errValue = `"TradeItemHasNoValue"`;
let _errXTime = `"ActionNowImproper"`;
////////////////////////////////////////////////////////////
let _warnPrgIdno = `Offer/Order ID must be a positive integer`;
let _warnPrgInit = `Offer/Order amount must be larger than 0`;
let _warnPrgNOwn = `You are not the owner of this offer/order`;
let _warnPrgPrix = `Tokens-per-coin price must be positive`;
let _warnPrgPrvt = `Coins-per-token price must be positive`;
let _warnPrgRate = `Coins-per-token rate is calculated below`;
let _warnPrgRvrt = `Tokens-per-coin rate is calculated above`;
let _warnPrgSAge = `Trade expiration must be in future time`;
let _warnPrgStop = `You are about to cancel this offer/order`;
////////////////////////////////////////////////////////////